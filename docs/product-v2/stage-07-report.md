...
## Revision 9 — замер отдельной модели-проверяющего (закрытие Stage 07)

Прогон владельца на `de63e13`, оба режима на одних и тех же вопросах (ответ — `qwen2.5-7b-instruct`).

| Режим | Обзор | Узкий вопрос |
|---|---|---|
| `same_model` (7B) | 99 с (llm 57 · проверка 28) | 56 с (llm 24 · проверка 31) |
| `separate_model` + `qwen3-4b-instruct-2507` | **51 с** (llm 37 · проверка 11) | **39 с** (llm 18 · проверка 21) |

Качество отдельной 4B не хуже, а лучше: на обзоре показано 3 утверждения против 2, на узком вопросе она подтвердила верный ответ, который та же 7B в роли проверяющего отклонила (`verifier:contradicted`). Правки Revision 8 дали дополнительное ускорение проверки (28 с против 51 с на той же 7B).

### Вывод
- Рекомендуемая настройка записана в `README.md`: `verifier.mode = separate_model` и небольшая отдельная модель. Это же подтверждает ADR-004: изолированный прогон одной модели — запасной вариант, а не целевой.
- Изолированный прогон одной модели остаётся рабочим, но медленнее и строже к верным утверждениям.
- Обзорный вопрос по-прежнему дороже узкого примерно вдвое; дальнейшее ускорение — вопрос выбора моделей на Stage 11.

Stage 07 закрыт: независимая проверка PASS (Revision 3), поведение с реальной моделью подтверждено.
