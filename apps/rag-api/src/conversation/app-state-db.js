...
// Applies numbered SQL migrations once each; re-running on an up-to-date database is a no-op.
export async function applyMigrations(db) {
  ...
  for (const migration of await migrationFiles()) {
    ...
    try {
      ...
      db.exec("ROLLBACK");
      throw new Error(`app-state migration ${migration.name} failed: ${error.message}`);
    }
  }
  return newlyApplied;
}
